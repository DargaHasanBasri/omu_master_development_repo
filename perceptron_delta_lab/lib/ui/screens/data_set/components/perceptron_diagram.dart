...
class PerceptronDiagram extends StatelessWidget {
  const PerceptronDiagram({super.key, this.strokeWidth = 2.0});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<DataSetTableCubit, DataSetTableState>(
      buildWhen: (p, n) => p.inputCount != n.inputCount,
      builder: (context, state) {
        return CustomPaint(
          painter: _PerceptronPainter(
            xCount: state.inputCount,
            stroke: strokeWidth,
...
class _PerceptronPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final biasTop = Offset(center.dx, topPad); // biraz aşağı aldık
    final biasEnd = Offset(center.dx, center.dy - radius - 2);
    _drawArrow(canvas, biasTop, biasEnd, p);

    _drawLabelCapsule(canvas, biasTop.translate(-6, -16), '1');
    _drawLabelCapsule(
      canvas,
  ...
  @override
  bool shouldRepaint(covariant _PerceptronPainter old) =>
      old.xCount != xCount ||
      old.stroke != stroke ||
      old.color != color ||
